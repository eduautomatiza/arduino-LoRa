...
int LoRaClass::endPacket(bool async, int implicitHeader) {
  if (isTransmitting()) {
    return 0;
  }
  ...
  // if (_crcEnabled) {
  //   compute_CRC16(_tx_payload, _tx_payload_length);
  //   fifoTransferWrite(_tx_payload, _tx_payload_length + 2);
  //   Serial.println("CRC included");
  // } else {
...
size_t LoRaClass::write(uint8_t byte) {
  if (_tx_payload_length >= MAX_PKT_LENGTH) {
    return 0;
  }
  _tx_payload[_tx_payload_length++] = byte;
...
size_t LoRaClass::write(const uint8_t* buffer, size_t size) {
  // check size
  if ((_tx_payload_length + size) > MAX_PKT_LENGTH) {
    size = MAX_PKT_LENGTH - _tx_payload_length;
  }
  // write data
  if (size) {
    // fifoTransferWrite(buffer, size);
    memcpy(&_tx_payload[_tx_payload_length], buffer, size);
    // update length
    _tx_payload_length += size;
  }
  return size;
}
...
int LoRaClass::read(uint8_t* buf, size_t size) {
